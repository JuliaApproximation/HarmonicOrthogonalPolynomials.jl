...
# work around for bad sphere eltype
Inclusion(::UnitSphere{SVector{3,T}}) where T = Inclusion(UnitSphere{SphericalCoordinate{T}}())
...
grammatrix(S::AbstractSphericalHarmonic{T}) where T = SquareEye{T}((axes(S,2),))
...
basis_axes(ax::Inclusion{<:Any,<:UnitSphere}, v::AbstractQuasiArray{T}) where T<:Real = RealSphericalHarmonic{float(T)}()
basis_axes(ax::Inclusion{<:Any,<:UnitSphere}, v::AbstractQuasiArray{T}) where T<:Complex = SphericalHarmonic{float(T)}()